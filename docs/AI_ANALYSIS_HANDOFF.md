...
0. For pre-match/scouting work, research current form, table position, fixture congestion, manager tenure, and well-reported injuries yourself (see `docs/PREMATCH_SCOUTING_CONTEXT.md`) rather than asking Michele to compile public information — but re-verify close to matchday, since this goes stale fast, and flag when coverage is thin. Only ask Michele for the genuinely non-public or creative pieces (corrections, insider info, article angle).
...
- No external context: competition importance, league table position, recent form, head-to-head history, injuries/team news. Most of this is public and should be researched via web search (see `docs/PREMATCH_SCOUTING_CONTEXT.md`) rather than inferred from the event data or asked of Michele; only the non-public/creative pieces (insider info, corrections, article angle) need to come from him.
