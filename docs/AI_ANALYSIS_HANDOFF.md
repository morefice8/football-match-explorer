...
5. Use `representative_sequences` (event-level x/y/outcome/receiver) to describe one or two concrete passages of play in prose — this is what turns a stats table into an actual tactical narrative ("in the 23rd move that built the goal, De Bruyne's through ball...").
...
- No cards/discipline section (yellow/red cards aren't broken out; a `dismissal` may appear only as a `formations.timeline` change reason).
...
See the project's task backlog (analyst-hat: xT model, shot-level xG, cards/discipline, game-state splits, possession %; dev-hat: pre-match pack mode) for planned fixes to the gaps listed above.
