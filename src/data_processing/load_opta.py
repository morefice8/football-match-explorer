    ...
    try:
        ...
        if start_pos != -1 and end_pos != -1 and end_pos > start_pos:
            ...
            try:
                ...
                logger.info(f"Successfully cleaned and loaded JSON from {file_path}")
                return data
            ...
            logger.warning(f"Error: Could not find valid JSON start/end braces in {file_path}.")
            return None
        ...
        logger.warning(f"An unexpected error occurred while cleaning JSON from {file_path}: {e}")
        return None