...
def process_opta_events(opta_data, event_mapping, qualifier_mapping, match_info, debug_excel_path=None):
    logger.debug("Starting Opta event processing...")
    ...
    if not event_details:
        logger.warning("Warning: No event data found. Returning empty DataFrame and empty dicts.")
        return pd.DataFrame(), {}, {}, {}
    ...
    logger.debug("Processing lineup/formation events (typeId 34)...")
    ...
    for team_name_key, formation_events_list in all_formation_events_by_team.items():
        ...
        except Exception as e: logger.warning(f"Error building INITIAL lineup metadata for team {team_id_for_meta}: {e}")
    logger.debug(f"Initial Team Formations: {initial_team_formations}")
    logger.debug(f"Initial Player Metadata sets: {len(initial_lineup_player_metadata)}")

    processed_events = []
    unique_qualifiers_found = set()
    logger.debug("Processing all game events...")
    ...
    if df.empty:
        logger.warning("Warning: DataFrame is empty after processing events.")
        return pd.DataFrame(), initial_lineup_player_metadata, initial_team_formations, all_formation_events_by_team

    logger.info(f"DataFrame created with shape: {df.shape}. Columns initially (sample): {df.columns.tolist()[:25]}...")
    logger.info(f"DEBUG: Unique qualifier IDs found in data: {sorted(list(str(qid) for qid in unique_qualifiers_found))}")

    logger.debug("Adding type names...")
    ...
    logger.debug("Renaming outcome...")
    if 'outcome' in df.columns: df['outcome'] = df['outcome'].map({0: 'Unsuccessful', 1: 'Successful'}).fillna('Unknown')
    else: logger.warning("Warning: 'outcome' column not found.")

    logger.debug("Renaming qualifier columns...")
    ...
    if qualifier_mapping and unique_qualifiers_found:
        for q_id_from_set in unique_qualifiers_found:
            ...
            if temp_col_name in df.columns:
                ...
                else: logger.warning(f"Warning: QID '{q_id_from_set}' (type: {type(q_id_from_set)}) not found in qualifier_mapping file.")
        if rename_dict: df.rename(columns=rename_dict, inplace=True)
        logger.debug(f"Renamed {qualifiers_renamed_count} qualifier columns out of {len(unique_qualifiers_found)} unique QIDs.")
        if qualifiers_renamed_count == 0 and unique_qualifiers_found: logger.warning(f"WARNING: No qualifier columns were renamed. DF columns: {df.columns.tolist()[:25]}")
    else:
        if not unique_qualifiers_found: logger.debug("Info: No unique qualifiers were found.")
        if not qualifier_mapping: logger.warning("Warning: Qualifier mapping empty.")

    # --- INIZIO BLOCCO CORRETTO E FINALE PER I FLAG ---
    logger.debug("Creating special pass flags (is_key_pass, is_assist)...")
    ...
    if TARGET_COL in df.columns:
        ...
        logger.info(f"  Flags created. Assists found: {df['is_assist'].sum()}. Key Passes found: {df['is_key_pass'].sum()}.")
    else:
        # Questo non dovrebbe succedere, ma è una sicurezza
        logger.warning(f"  CRITICAL WARNING: Column '{TARGET_COL}' not found. Cannot create key pass/assist flags.")
    ...
    logger.debug(f"Applied {len(actual_renames_to_apply)} common renames.")
    ...
    for col_name in known_numeric_renamed_qualifiers:
        if col_name in df.columns:
            ...
            logger.debug(f"Attempted conversion of column '{col_name}' to numeric.")
    ...
    for col_name in flag_like_renamed_columns:
        if col_name in df.columns:
            ...
            logger.info(f"Processed flag-like column '{col_name}' to 0/1 int.")
    ...
    for col_name in columns_to_ensure_string:
        if col_name in df.columns:
            logger.debug(f"Ensuring column '{col_name}' is string type before saving.")
    ...
    logger.debug("Removing unwanted events...")
    values_to_remove = ['Deleted event']; original_rows = len(df)
    if 'type_name' in df.columns: df = df[~df['type_name'].isin(values_to_remove)].copy(); logger.debug(f"Removed {original_rows - len(df)} rows.")
    else: logger.warning("Warning: 'type_name' column not found for filtering.")

    logger.debug("Creating shorter names...")
    if 'playerName' in df.columns: df['shorter_name'] = df['playerName'].apply(get_shorter_name)
    else: logger.warning("Warning: 'playerName' column not found.")

    logger.debug("Dropping duplicate events based on 'id'...")
    if 'id' in df.columns:
        original_rows = len(df); df.drop_duplicates(subset='id', inplace=True, keep='first'); logger.debug(f"Removed {original_rows - len(df)} duplicate rows.")
    else: logger.warning("Warning: 'id' column not found for deduplication.")

    if 'end_x' not in df.columns or 'end_y' not in df.columns: logger.warning("Warning: 'end_x' or 'end_y' columns missing.")
    if 'positional_role' not in df.columns: logger.warning("Warning: 'positional_role' column failed to be added.")
    elif 'positional_role' in df.columns and df['positional_role'].isnull().all(): logger.warning("Warning: 'positional_role' column added but all values are Null/Unknown.")

    logger.info(f"Preprocessing complete. Final DataFrame shape: {df.shape}")
    logger.debug(f"Final Columns (sample): {df.columns.tolist()[:25]}...")
    # For debugging the problematic 'Foul' column if the error persists:
    if 'Foul' in df.columns:
        logger.debug(f"DEBUG: 'Foul' column dtype: {df['Foul'].dtype}")
        logger.debug(f"DEBUG: Unique values in 'Foul' column: {df['Foul'].unique()[:20]}") # Print some unique values
    ...
    if debug_excel_path:
        try:
            logger.debug(f"Attempting to save debug Excel file to: {debug_excel_path}")
            ...
            logger.info(f"Successfully saved debug Excel file: {debug_excel_path}")
        except ImportError: logger.warning("Error saving Excel: `openpyxl` library not found.")
        except Exception as e: logger.warning(f"Error saving debug Excel file: {e}")
