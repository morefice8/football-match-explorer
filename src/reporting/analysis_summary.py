...
def _shots(bundle) -> list[dict[str, Any]]:
    ...
    Reuses the canonical ``overview.shots`` frame (already produced by
    ...
    data = _section_data(bundle, "overview")
    frame = (
        _as_frame(data.get("shots")) if isinstance(data, Mapping) else pd.DataFrame()
    )
    if frame.empty:
        return []
        ...
        rows.append(
            {
                ...
                "x": _safe_number(_row_value(row, frame, "x")),
                "y": _safe_number(_row_value(row, frame, "y")),
                "outcome": _row_value(row, frame, "shot_outcome"),
                "on_target": _row_value(row, frame, "shot_on_target"),
                "blocked": _row_value(row, frame, "shot_blocked"),
