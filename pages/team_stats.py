...
def get_available_seasons():
    """Scans the data directory to find all available seasons."""
    seasons = set()
...
def create_league_filter_buttons():
    ...
    buttons = [
        dbc.Button(html.I(className="fas fa-globe fa-2x"), id="filter-all", n_clicks=0, color="primary", outline=True, style=button_style),
        dbc.Tooltip("Show All Leagues", target="filter-all")
    ]
    for league_folder, league_info in LEAGUES.items():
        button_id = f"filter-{league_folder}"
        buttons.append(
            dbc.Button(html.Img(src=league_info['logo'], style=logo_style), id=button_id, n_clicks=0, color="secondary", outline=True, style=button_style)
        )
...
def create_metric_card(df, title, metric_column, ascending, unit="", format_spec="{:,.1f}", icon=""):
    ...
    card_header = dbc.CardHeader(
        dbc.Row([
            dbc.Col(html.I(className=f"{icon} me-2"), width="auto"),
            dbc.Col(html.H5(title, className="m-0"), width=True),
        ], align="center"), id=header_id, className="text-center"
    )
...
def generate_tabs_content(df, selected_league='All'):
    ...
    return dbc.Tabs(
        ...
        children=[
            dbc.Tab(label="Attacking", tab_id="tab-attacking", children=html.Div(dbc.Row([create_metric_card(df_filtered, **m) for m in attacking_metrics]), className="p-2 mt-3")),
            dbc.Tab(label="Possession & Style", tab_id="tab-possession", children=html.Div(dbc.Row([create_metric_card(df_filtered, **m) for m in possession_metrics]), className="p-2 mt-3")),
            dbc.Tab(label="Defending", tab_id="tab-defending", children=html.Div(dbc.Row([create_metric_card(df_filtered, **m) for m in defensive_metrics]), className="p-2 mt-3")),
        ], className="mt-4"
    )
...
def layout():
    ...
    if default_season:
        df_teams = load_all_team_stats(default_season)
        initial_content = generate_tabs_content(df_teams, 'All')
    else:
        df_teams = pd.DataFrame()
        initial_content = dbc.Alert("No season data found.", color="danger")
    return dbc.Container([
        dcc.Store(id='team-stats-full-df-store', data=df_teams.to_json(orient='split')),
        ...
        html.Div(id="team-stats-tabs-content", children=initial_content),
        html.Div(id="league-quadrant-plots", className="mt-5")
    ], fluid=True, className="p-4")

@callback(
    Output('team-stats-full-df-store', 'data'),
    Output('active-league-filter-store', 'data', allow_duplicate=True),
...
def update_store_on_season_change(selected_season):
    if not selected_season:
        return None, 'All'
    print(f"--- Season changed to {selected_season}. Reloading data. ---")
    df = load_all_team_stats(selected_season)
    return df.to_json(orient='split'), 'All'
...
def update_quadrant_plots(selected_league, df_json):
    if not df_json or selected_league == 'All':
        # Non mostrare i grafici se non è selezionata una lega specifica
        return None
    ...
    # Definiamo i parametri per ogni grafico
    plot_definitions = [
        {
            "title": "Goal Conversion vs. Total Shots", "x_metric": "Sh", "y_metric": "Goal_Conversion",
            "x_label": "Total Shots", "y_label": "Goal Conversion %", "invert_y": False,
            "quadrant_labels": ['Elite Attack', 'Wasteful Attack', 'Ineffective Attack', 'Clinical Attack']
        },
        {
            "title": "Short vs. Long Ratio vs. Passing Tempo", "x_metric": "Passing_Tempo", "y_metric": "Short_vs_Long_Ratio",
            "x_label": "Passing Tempo", "y_label": "Short vs Long Ratio", "invert_y": False,
            "quadrant_labels": ['Fast & Short', 'Fast & Direct', 'Slow & Direct', 'Slow & Methodical']
        },
        {
            "title": "Shots Conceded per DA vs. Defensive Actions", "x_metric": "Defensive_Actions", "y_metric": "Shots_Conceded_per_DA",
            "x_label": "Defensive Actions", "y_label": "Shots Conceded per DA", "invert_y": True,
            "quadrant_labels": ['Proactive & Solid', 'Busy & Leaky', 'Passive & Vulnerable', 'Organized & Efficient']
        }
    ]

    quadrant_graphs = [
        dbc.Col(dcc.Graph(figure=league_plots.create_quadrant_plot(df_league, **p)), lg=12, className="mb-4")
        for p in plot_definitions
    ]

    return dbc.Container([
        html.H2(f"{selected_league} - Quadrant Analysis", className="text-white text-center mb-4"),
        dbc.Row(quadrant_graphs)
    ], fluid=True)


# --- END OF FILE team_stats.py ---